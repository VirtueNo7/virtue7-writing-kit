# Worked example: Job-to-Resume Handoff

## User request

Pass one selected role into Resume as a compact evidence packet without carrying the search history.
...
**Route contract applied:** Create one compact job evidence packet containing verified posting facts, requirements, keywords, uncertainties, fit rationale, and recommended resume emphasis. Then close Job Search before loading Resume.
...
- Boot: three files
- Route packet: `runtime/packets/routes/job-search--handoff.md` (4154 bytes)
...
```yaml
artifact_id: 11-job-search-handoff
route: job-search/handoff
profile: resume
